...
import net.seabears.campsites.domain.DateAvailability;
...
public class AvailabilityControllerIT {
    ...
    public void getCampsite() throws Exception {
        ...
        final ResponseEntity<CampgroundAvailability> response = template.getForEntity(url, CampgroundAvailability.class, id, start, end);
    ...
    public void getCampsites(final String resource) throws Exception {
        ...
        final LocalDate start = LocalDate.now();
        final LocalDate end = start.plusDays(days);
        final ResponseEntity<CampgroundAvailability> response = template.getForEntity(url, CampgroundAvailability.class, resource, id, start, end);
        final CampgroundAvailability availability = response.getBody();
